...
(function(global) {
  ...
  global.WBData = {
    ...
    getResult(id) {
      ...
      if (!/^\d{4}$/.test(r)) return '----';
      // Validasi tanggal: result hanya valid kalau history[0].date === hari ini
      const today = (_data && _data._meta && _data._meta.today_str)
        || new Date().toISOString().slice(0, 10);
      const histDate = e.history && e.history[0] && e.history[0].date;
      if (histDate && histDate !== today) return '----';
      return r;
    },
